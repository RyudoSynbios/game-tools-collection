...
export function randomizeParty(
  ...
  for (let i = 0x0; i < mainDolModels.party.count; i += 0x1) {
    ...
    if (options.party.stats !== 0x0) {
      let mp = getInt(offset + 0xf, "uint8", {}, $dataViewAlt.party); // prettier-ignore
      let hp = getInt(offset + 0x1c, "uint16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let power = getInt(offset + 0x60, "uint16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let will = getInt(offset + 0x62, "uint16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let vigor = getInt(offset + 0x64, "uint16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let agile = getInt(offset + 0x66, "uint16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let quick = getInt(offset + 0x68, "uint16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthHp = getInt(offset + 0x1e, "int16", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthMp = getInt(offset + 0x2c, "float32", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthPower = getInt(offset + 0x6c, "float32", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthWill = getInt(offset + 0x70, "float32", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthVigor = getInt(offset + 0x74, "float32", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthAgile = getInt(offset + 0x78, "float32", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      let growthQuick = getInt(offset + 0x7c, "float32", { bigEndian: true }, $dataViewAlt.party); // prettier-ignore
      ...
      if (options.party.stats === 0x1) {
        ...
        growthPower *= prng.getFloat(0.5, 1.5, `party_stats_growthPower_1_${i}`); // prettier-ignore
        growthWill *= prng.getFloat(0.5, 1.5, `party_stats_growthWill_1_${i}`);
        growthVigor *= prng.getFloat(0.5, 1.5, `party_stats_growthVigor_1_${i}`); // prettier-ignore
        growthAgile *= prng.getFloat(0.5, 1.5, `party_stats_growthAgile_1_${i}`); // prettier-ignore
        growthQuick *= prng.getFloat(0.5, 1.5, `party_stats_growthQuick_1_${i}`); // prettier-ignore
      ...
      setInt(offset + 0x1a, "uint16", hp, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x1c, "uint16", hp, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x60, "uint16", power, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x62, "uint16", will, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x64, "uint16", vigor, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x66, "uint16", agile, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x68, "uint16", quick, { bigEndian: true }, 'party'); // prettier-ignore
      setInt(offset + 0x1e, "int16", growthHp, { bigEndian: true }, "party"); // prettier-ignore
      setInt(offset + 0x2c, "float32", growthMp, { bigEndian: true }, "party"); // prettier-ignore
      setInt(offset + 0x6c, "float32", growthPower, { bigEndian: true }, "party"); // prettier-ignore
      setInt(offset + 0x70, "float32", growthWill, { bigEndian: true }, "party"); // prettier-ignore
      setInt(offset + 0x74, "float32", growthVigor, { bigEndian: true }, "party"); // prettier-ignore
      setInt(offset + 0x78, "float32", growthAgile, { bigEndian: true }, "party"); // prettier-ignore
      setInt(offset + 0x7c, "float32", growthQuick, { bigEndian: true }, "party"); // prettier-ignore
    }
    ...
    if (options.party.magic === 0x1 || options.qualityOfLife.magicUnlocked) {
      ...
      for (let j = 0x0; j < 0x24; j += 0x1) {
        ...
        experiences.push(
          getInt(offset, "uint16", { bigEndian: true }, $dataViewAlt.experienceCurves), // prettier-ignore
        );
      }
    ...
    if (options.party.equipment === 0x1) {
      ...
      const weaponIndex = prng.getInt(0, weapons.length - 1, `party_weapon_${i}`); // prettier-ignore
      const armorIndex = prng.getInt(0, armors.length - 1, `party_armor_${i}`); // prettier-ignore
      const accessoryIndex = prng.getInt(0, accessories.length - 1, `party_accessory_${i}`); // prettier-ignore
