...
export function randomizeShops(
  ...
  for (let i = 0x0; i < mainDolModels.shops.count; i += 0x1) {
    ...
    if (options.shops.count === 0x0) {
      ...
      const value = prng.getInt(0, filteredShops.length - 1, `shops_count_1_${i}`); // prettier-ignore
    ...
    for (let j = 0x0; j < 0x30; j += 0x1) {
      ...
      if (options.shops.items === 0x1 || options.shops.items === 0x2) {
        ...
        const value = prng.getInt(0, filteredItems.length - 1, `shops_item_${options.shops.items}_${j}_${i}`); // prettier-ignore
    ...
    items.forEach((item, index) => {
      setInt(offset + 0x8 + index * 0x2, "uint16", item, { bigEndian: true }, "shops"); // prettier-ignore
    });
  }
}
