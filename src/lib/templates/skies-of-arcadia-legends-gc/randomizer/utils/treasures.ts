...
export function randomizeTreasures(
  ...
  if (options.treasures.contents === 0x1) {
    ...
    for (let i = 0x0; i < mainDolModels.treasures.count; i += 0x1) {
      ...
      const itemIndex = getInt(offset, 'uint32', { bigEndian: true }, $dataViewAlt.treasures); // prettier-ignore
      const quantity = getInt(offset + 0x4, 'uint32', { bigEndian: true }, $dataViewAlt.treasures); // prettier-ignore
    ...
    for (let i = 0x0; i < mainDolModels.treasures.count; i += 0x1) {
      ...
      setInt(offset, 'uint32',treasures[i].itemIndex, { bigEndian: true }, 'treasures'); // prettier-ignore
      setInt(offset + 0x4, 'uint32',treasures[i].quantity, { bigEndian: true }, 'treasures'); // prettier-ignore
    }
  } else if (options.treasures.contents === 0x2) {
    for (let i = 0x0; i < mainDolModels.treasures.count; i += 0x1) {
      const offset = i * mainDolModels.treasures.length;

      const itemIndex = prng.getInt(0, inventory.length - 1, `treasures_contents_2_${i}`); // prettier-ignore
      ...
      setInt(offset, 'uint32',item.index, { bigEndian: true }, 'treasures'); // prettier-ignore
      setInt(offset + 0x4, 'uint32',quantity, { bigEndian: true }, 'treasures'); // prettier-ignore
    }
  }
}
