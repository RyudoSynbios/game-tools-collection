...
export function randomizeEnemies(
  ...
  for (let i = 0x0; i < 0xff; i += 0x1) {
    ...
    if (options.enemies.stats !== 0x0) {
      let magicAttribute = getInt(offset + 0x17, "uint8", {}, $dataViewAlt.enemies); // prettier-ignore
      let hp = getInt(offset + 0x24, "uint32", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let will = getInt(offset + 0x5e, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let vigor = getInt(offset + 0x60, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let agile = getInt(offset + 0x62, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let quick = getInt(offset + 0x64, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let attack = getInt(offset + 0x66, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let defense = getInt(offset + 0x68, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let magicDefense = getInt(offset + 0x6a, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let hit = getInt(offset + 0x6c, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      let dodge = getInt(offset + 0x6e, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore

      if (options.enemies.stats === 0x1) {
        magicAttribute *= prng.getFloat(0.5, 1.5, `enemies_enemy_magicAttribute_1_${i}`); // prettier-ignore
        ...
        magicDefense *= prng.getFloat(0.5, 1.5, `enemies_enemy_magicDefense_1_${i}`); // prettier-ignore
        ...
        magicDefense *= prng.getFloat(0.1, 4, `enemies_enemy_magicDefense_2_${i}`); // prettier-ignore
      ...
      setInt(offset + 0x6a, "uint16", magicDefense, { bigEndian: true }, "enemies"); // prettier-ignore
    ...
    let experience = getInt(offset + 0x1e, 'uint16', { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
    let gold = getInt(offset + 0x20, 'uint16', { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
    ...
    if (options.enemies.drops === 0x1) {
      const countRate = prng.getInt(0, itemCountRates.length - 1, `enemies_enemy_countRate_${i}`); // prettier-ignore
      const count = itemCountRates[countRate];

      // Check if first drop is a key item
      const rate = getInt(offset + 0x72, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      const itemIndex = getInt(offset + 0x76, "uint16", { bigEndian: true }, $dataViewAlt.enemies); // prettier-ignore
      ...
      if (rate !== 100 || item) {
        for (let j = 0x0; j < 0x4; j += 0x1) {
          ...
          if (j < count) {
            const itemIndex = prng.getInt(0, items.length - 1, `enemies_enemy_dropItem_${j}_${i}`); // prettier-ignore
            const rateRangeIndex = prng.getInt(0, itemRateRanges.length - 1, `enemies_enemy_rateRange_${j}_${i}`); // prettier-ignore

            const { min, max } = itemRates[itemRateRanges[rateRangeIndex]];

            item = items[itemIndex].index;
            quantity = 1;
            rate = prng.getInt(min, max, `enemies_enemy_rate_${j}_${i}`); // prettier-ignore
          }

          setInt(offset + j * 0x6 + 0x76, "uint16", item, { bigEndian: true }, 'enemies'); // prettier-ignore
          setInt(offset + j * 0x6 + 0x74, "uint16", quantity, { bigEndian: true }, 'enemies'); // prettier-ignore
          setInt(offset + j * 0x6 + 0x72, "uint16", rate, { bigEndian: true }, 'enemies'); // prettier-ignore
        }
      }
    }
  }
  ...
  for (let i = 0x0; i < mainDolModels.enemyShips.count; i += 0x1) {
    ...
    if (options.enemies.stats !== 0x0) {
      let hp = getInt(offset + 0x14, "uint32", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
      let will = getInt(offset + 0x18, "uint16", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
      let defense = getInt(offset + 0x1a, "uint16", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
      let magicDefense = getInt(offset + 0x1c, "uint16", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
      let quick = getInt(offset + 0x1e, "uint16", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
      let agile = getInt(offset + 0x20, "uint16", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
      let dodge = getInt(offset + 0x22, "uint16", { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore

      if (options.enemies.stats === 0x1) {
        hp *= prng.getFloat(0.5, 1.5, `enemies_ship_hp_1_${i}`); // prettier-ignore
        will *= prng.getFloat(0.5, 1.5, `enemies_ship_will_1_${i}`);
        defense *= prng.getFloat(0.5, 1.5, `enemies_ship_defense_1_${i}`);
        magicDefense *= prng.getFloat(0.5, 1.5, `enemies_ship_magicDefense_1_${i}`); // prettier-ignore
        ...
        hp *= prng.getFloat(0.1, 4, `enemies_ship_hp_2_${i}`); // prettier-ignore
        will *= prng.getFloat(0.1, 4, `enemies_ship_will_2_${i}`);
        defense *= prng.getFloat(0.1, 4, `enemies_ship_defense_2_${i}`);
        magicDefense *= prng.getFloat(0.1, 4, `enemies_ship_magicDefense_2_${i}`); // prettier-ignore
      ...
      setInt(offset + 0x1a, "uint16", defense, { bigEndian: true }, "enemyShips"); // prettier-ignore
      setInt(offset + 0x1c, "uint16", magicDefense, { bigEndian: true }, "enemyShips"); // prettier-ignore
    ...
    let experience = getInt(offset + 0x64, 'int32', { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
    let gold = getInt(offset + 0x68, 'int32', { bigEndian: true }, $dataViewAlt.enemyShips); // prettier-ignore
    ...
    setInt(offset + 0x64, 'uint32', experience, { bigEndian: true }, 'enemyShips'); // prettier-ignore
    setInt(offset + 0x68, "uint32", gold, { bigEndian: true }, "enemyShips");

    // Drops

    if (options.enemies.drops === 0x1) {
      const countRate = prng.getInt(0, shipItemCountRates.length - 1, `enemies_ship_countRate_${i}`); // prettier-ignore
      ...
      for (let j = 0x0; j < 0x3; j += 0x1) {
        ...
        if (j < count) {
          const itemIndex = prng.getInt(0, shipItems.length - 1, `enemies_ship_dropItem_${j}_${i}`); // prettier-ignore
          const rateIndex = prng.getInt(0, shipIemRates.length - 1, `enemies_ship_rateRange_${j}_${i}`); // prettier-ignore

          item = shipItems[itemIndex].index;
          rate = shipIemRates[rateIndex];
        }

        setInt(offset + j * 0x4 + 0x6e, "uint16", item, { bigEndian: true }, 'enemyShips'); // prettier-ignore
        setInt(offset + j * 0x4 + 0x6c, "uint16", rate, { bigEndian: true }, 'enemyShips'); // prettier-ignore
      }
    }
  }
}
