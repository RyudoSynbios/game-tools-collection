...
export function randomizeAbilities(prng: Prng, options: Options): void {
  ...
  for (let i = 0x0; i < mainDolModels.abilities.count; i += 0x1) {
    ...
    if (
      ...
      let effectValue = getInt(offset + 0x1c, "int16", { bigEndian: true }, $dataViewAlt.abilities); // prettier-ignore
      let shipEffectValue = getInt(offset + 0x2a, "int16", { bigEndian: true }, $dataViewAlt.abilities); // prettier-ignore
      ...
      if (
        ...
        if (effectValue > 0) {
          effectValue *= prng.getFloat(0.5, 1.5, `abilities_effectValue_1_${i}`); // prettier-ignore
        }

        if (shipEffectValue > 0) {
          shipEffectValue *= prng.getFloat(0.5, 1.5, `abilities_shipEffectValue_1_${i}`); // prettier-ignore
        }
      } else if (
        ...
        if (effectValue > 0) {
          effectValue *= prng.getFloat(0.1, 4, `abilities_effectValue_2_${i}`); // prettier-ignore
        }

        if (shipEffectValue > 0) {
          shipEffectValue *= prng.getFloat(0.1, 4, `abilities_shipEffectValue_2_${i}`); // prettier-ignore
        }
      }

      setInt(offset + 0x1c, "int16", effectValue, { bigEndian: true }, 'abilities'); // prettier-ignore
      setInt(offset + 0x2a, "int16", shipEffectValue, { bigEndian: true }, 'abilities'); // prettier-ignore
    }
  ...
  if (options.abilities.magicRequiredRanks === 0x1) {
    ...
    for (let i = 0x0; i < abilityTypes[0].count; i += 0x1) {
      ...
      const magic = getInt(offset, "uint8", {}, $dataViewAlt.experienceCurves); // prettier-ignore
      const rank = getInt(offset + 0x1, "uint8", {}, $dataViewAlt.experienceCurves) - 0x1; // prettier-ignore

      setInt(offset + 0x1, "uint8", ranks[magic][rank], {}, 'experienceCurves'); // prettier-ignore
    }
    ...
    for (let i = 0x0; i < abilityTypes[0].count; i += 0x1) {
      setInt(0xaf8 + i * 0x2, "uint16", ranks[i], { bigEndian: true }, 'experienceCurves'); // prettier-ignore
    }
  }
  ...
  if (options.abilities.superMoveRequiredMoonberries !== 0x0) {
    ...
    for (let i = 0x0; i < abilityTypes[1].count; i += 0x1) {
      ...
      const character = getInt(offset, "uint8", {}, $dataViewAlt.experienceCurves); // prettier-ignore
      const moonberries = getInt(offset + 0x1, "uint8", {}, $dataViewAlt.experienceCurves); // prettier-ignore
    ...
    if (options.abilities.superMoveRequiredMoonberries === 0x2) {
      characters.forEach((character, index) => {
        character.map((moonstones) =>
          Math.round(
            moonstones * prng.getFloat(0.1, 4, `abilities_moonberries_2_${moonstones}_${index}`), // prettier-ignore
          ),
        );
      });
    }
    ...
    for (let i = 0x0; i < abilityTypes[1].count; i += 0x1) {
      ...
      setInt(offset + 0x1, "uint8", moonberries, {}, 'experienceCurves'); // prettier-ignore
    }
  }
