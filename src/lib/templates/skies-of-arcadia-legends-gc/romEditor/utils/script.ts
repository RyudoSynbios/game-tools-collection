...
export default class Script {
      ...
      if (
        ...
        switch (instruction) {
          ...
          case 0x27:
            ...
            break;
          case 0x28:
            this.parseSetCameraToEntity(action);
            break;
          case 0x29:
          ...
          case 0x50:
            ...
            break;
          case 0x51:
            this.parseUnknown51(action);
            break;
          case 0x52:
            this.parseAmbientLight(action);
            break;
          case 0x53:
            this.parseDirectionalLight(action);
            break;
          case 0x54:
          ...
          case 0x82:
            ...
            break;
          case 0x83:
            this.parseUnknown83(action);
            break;
          case 0x84:
            this.parseUnknown84(action);
            break;
          case 0x85:
            this.parseUnknown85(action);
            break;
          case 0x86:
  ...
  private parseVariables(action: Action, isFlag = false): string {
    ...
    if (variables.length === 2) {
      debug.warn(variables.length, texts.join(""), action);
    }
  ...
  private parseSetCameraToEntity(action: Action): void {
    const entity = this.parseVariables(action);
    const lookAtY = this.parseVariables(action);
    const isAnimated = this.parseVariables(action);

    action.color = "limegreen";
    action.text = `set camera to look at entity "${entity}" with Y = "${lookAtY}" (${isAnimated === "0" ? "not " : ""}animated)`;
  }
  ...
  private parseSetCameraPosition(action: Action): void {
    ...
    action.color = "mediumorchid";
    action.text = `set camera position: X = "${unknown1}", Y = "${unknown2}", Z = "${unknown3}", Unknown = "${unknown4}"`;
  }
  ...
  private parseMoveCamera(action: Action): void {
    ...
    action.color = "mediumorchid";
    action.text = `move camera with values "${unknown1}", "${unknown2}", "${unknown3}", "${unknown4}", "${unknown5}", "${unknown6}", "${unknown7}"`;
  }
  ...
  private parseSetPlayerPosition(action: Action): void {
    ...
    action.color = "mediumorchid";
    action.text = `set player position: Unknown = "${unknown1}", X = "${unknown2}", Y = "${unknown3}", Z = "${unknown4}", Rotation = "${unknown5}°"`;
  }
  ...
  private parseFog(action: Action): void {
    ...
    action.color = "mediumorchid";
    action.text = `set fog: RGB = "[${red}, ${blue}, ${green}]", Duration = "${transitionDuration}", Near = "${near}", Far = "${far}", Unknown = "${unknown}"`;
  }
  ...
  private parseUnknown51(action: Action): void {
    const unknown1 = this.parseVariables(action);
    const unknown2 = this.parseVariables(action);
    const unknown3 = this.parseVariables(action);
    const unknown4 = this.parseVariables(action);
    ...
    action.color = "mediumorchid";
    action.text = `unknown instruction 0x51 with values "${unknown1}", "${unknown2}", "${unknown3}", "${unknown4}", "${unknown5}", "${unknown6}", "${unknown7}", "${unknown8}", "${unknown9}"`;
  }

  private parseAmbientLight(action: Action): void {
    action.text = "should set ambient light";
  ...
  private parseDirectionalLight(action: Action): void {
    const enabled = this.parseVariables(action);
    const unknown2 = this.parseVariables(action);
    const unknown3 = this.parseVariables(action);
    const unknown4 = this.parseVariables(action);
    ...
    action.color = "mediumorchid";
    action.text = `set directional light: Enabled = "${enabled}", Unknown = "${unknown2}", Unknown = "${unknown3}", Unknown = "${unknown4}", RGB = "[${red}, ${blue}, ${green}]"`;
  }
  ...
  private parseUnknown54(action: Action): void {
    ...
    action.color = "mediumorchid";
    action.text = `unknown instruction 0x54 with values "${unknown1}", "${unknown2}", "${unknown3}", "${unknown4}", "${unknown5}", "${unknown6}", "${unknown7}", "${unknown8}", "${unknown9}", "${unknown10}"`;
  }
  ...
  private parseUnknown6d(action: Action): void {
    ...
    action.color = "mediumorchid";
    action.text = `unknown instruction 0x6d with values "${unknown1}", "${unknown2}", "${unknown3}", "${unknown4}", "${unknown5}", "${unknown6}", "${unknown7}", "${unknown8}", "${unknown9}"`;
  }
  ...
  private parseUnknown83(action: Action): void {
    const count = this.getValue(action);
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      text += ` | "${unknown1}", "${unknown2}", "${unknown3}", "${unknown4}"`;
    }

    action.color = "mediumorchid";
    action.text = `unknown instruction 0x83 with ${count} block(s) ${text.replace(" | ", "")}`;
  }

  private parseUnknown84(action: Action): void {
    const count = this.getValue(action);
    ...
    action.color = "mediumorchid";
    action.text = `unknown instruction 0x84 with ${count} block(s) ${text.replace(" | ", "")}`;
  }

  private parseUnknown85(action: Action): void {
    const count = this.getValue(action);
    ...
    action.color = "mediumorchid";
    action.text = `unknown instruction 0x85 with value "${unknown}" then ${count} block(s) ${text.replace(" | ", "")}`;
  }
