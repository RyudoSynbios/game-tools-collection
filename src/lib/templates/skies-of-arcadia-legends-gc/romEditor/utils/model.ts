...
export function getVertices(
  ...
  while (true) {
    const size = getInt(offset, "uint16", { bigEndian: true }, dataView) * 0x4; // prettier-ignore
    const unknown2 = getInt(offset + 0x2, "uint8", {}, dataView); // prettier-ignore
    const type = getInt(offset + 0x3, "uint8", {}, dataView); // prettier-ignore
    ...
    for (let i = 0x0; i < count; i += 0x1) {
      ...
      const vX = getInt(offset, "float32", { bigEndian: true }, dataView); // prettier-ignore
      const vY = getInt(offset + 0x4, "float32", { bigEndian: true }, dataView); // prettier-ignore
      const vZ = getInt(offset + 0x8, "float32", { bigEndian: true }, dataView); // prettier-ignore

      offset += 0xc;

      if (hasNormals) {
        // const nX = getInt(offset + 0xc, "float32", { bigEndian: true }, dataView); // prettier-ignore
        // const nY = getInt(offset, "float32", { bigEndian: true }, dataView); // prettier-ignore
        // const nZ = getInt(offset + 0x4, "float32", { bigEndian: true }, dataView); // prettier-ignore

        offset += 0xc;
      }

      if (hasVertexColor) {
        // const vcRed = getInt(offset, "uint8", {}, dataView); // prettier-ignore
        // const vcGreen = getInt(offset + 0x1, "uint8", {}, dataView); // prettier-ignore
        // const vcBlue = getInt(offset + 0x2, "uint8", {}, dataView); // prettier-ignore
        // const vcAlpha = getInt(offset + 0x3, "uint8", {}, dataView); // prettier-ignore
...
export function addMeshs(
  ...
  while (true) {
    const flags = getInt(offset, "uint8", {}, dataView); // prettier-ignore
    const type = getInt(offset + 0x1, "uint8", {}, dataView); // prettier-ignore
    ...
    if (isSpecial1) {
      ...
      //     const type = getInt(objects[i].meshsOffset + 0x1, "uint8", {}, dataView); // prettier-ignore
    ...
    if (isMesh) {
      ...
      for (let i = 0x0; i < meshCount; i += 0x1) {
        ...
        for (let j = 0x0; j < indiceCount; j += 0x1) {
          ...
          if (hasUvs) {
            const uvX = getInt(offset, "int16", { bigEndian: true }, dataView) / uvSize; // prettier-ignore
            const uvY = getInt(offset + 0x2, "int16", { bigEndian: true }, dataView) / uvSize; // prettier-ignore
