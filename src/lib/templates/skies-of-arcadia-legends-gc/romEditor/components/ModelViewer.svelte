<script lang="ts">
  ...
  async function updateCanvas(): Promise<void> {
    ...
    three.addGuiElement("entityIndex", "Entity", entityIndex, (value) => { entityIndex = value; }, list); // prettier-ignore
    ...
    if (entity.linkedNjcmFiles.length > 1) {
      ...
      three.addGuiElement("njcmIndex", "NJCM", njcmIndex, (value) => { njcmIndex = value; }, list); // prettier-ignore
    }
    ...
    njcm.objects
      // .filter((o, index) => index === 1) // TODO: Remove
