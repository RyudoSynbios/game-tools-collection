<script lang="ts">
  ...
  async function updateCanvas(): Promise<void> {
    ...
    if (entities.length === 0) {
      ...
      three.addGuiListElement("entityIndex", "Entity", entityIndex, (value) => { entityIndex = value; }, list); // prettier-ignore
    ...
    await entities.reduce(async (previousEntity, entityIndex) => {
      ...
      if (entity.linkedNjcmFiles.length > 1) {
        ...
        three.addGuiListElement("njcmIndex", "NJCM", njcmIndex, (value) => { njcmIndex = value; }, list); // prettier-ignore
      }
