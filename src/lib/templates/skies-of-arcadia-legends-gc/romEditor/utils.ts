...
export function overrideGetInt(
  ...
  if ("id" in item && item.id?.match(/reversePercent-/)) {
    ...
    let int = getInt(itemInt.offset, 'uint8', {}, $dataViewAlt[dataViewAltKey]); // prettier-ignore
...
export function getWeaponConditionNames(): Resource {
  ...
  for (let i = 0x0; i < mainDolModels.weaponConditions.count; i += 0x1) {
    ...
    let hit = getInt(offset + 0x13, 'uint8', {}, $dataViewAlt.weaponConditions); // prettier-ignore
