...
export function getImagesCanvas(
  ...
  if (file.name.match(/^FACE64(.*?).DAT$/)) {
    ...
    imagesCanvas.images = parseSpriteFile(file);
...
function parseFace32File(file: File & { dataView: DataView }): Image[] {
  const images: Image[] = [];
...
function parseKaoFile(file: File & { dataView: DataView }): Image[] {
  const images: Image[] = [];
...
function parseSpriteFile(file: File & { dataView: DataView }): Image[] {
  const images: Image[] = [];
