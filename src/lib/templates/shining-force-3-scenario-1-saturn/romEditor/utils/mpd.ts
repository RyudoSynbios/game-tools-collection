...
import { applyPalette, flipUvs } from "$lib/utils/graphics";
...
async function getTextures(
  ...
  const textureCount = (decompressedData[0x0] << 0x8) | decompressedData[1];
  ...
  if (textureCount > 200) {
    ...
    for (let j = 0x0; j < textureCount; j += 0x1) {
      ...
      for (let j = 0x0; j < texture.length; j += 0x1) {
        const color = (texture[j * 2] << 0x8) | texture[j * 2 + 1];
...
export async function unpackMpd(
  ...
  if (paletteEndOffset - paletteStartOffset > 0) {
    for (let i = paletteStartOffset; i < paletteEndOffset; i += 0x2) {
      const color = getInt(i, "uint16", { bigEndian: true }, dataView);

      const red = (color >> 0xa) << 0x3;
      const green = ((color >> 0x5) & 0x1f) << 0x3;
      const blue = (color & 0x1f) << 0x3;
      const alpha = 255;

      mpd.palette.push([blue, green, red, alpha]);
    }
  }
