...
export function overrideParseItem(item: Item): Item {
  ...
  if (
    ...
    const [gfIndex, abilityIndex] = item.id.splitInt();

    const learnableAbilityIndex = learnableAbilities[gfIndex].findIndex(
      (index) => index === abilityIndex,
    );

    if (learnableAbilityIndex === -1) {
      return itemSection;
    }

    const apItem = itemSection.items[0] as ItemInt;
    const statusItem = itemSection.items[1] as ItemInt;

    apItem.offset += learnableAbilityIndex;
    apItem.max = gfAbilityList.find(
      (ability) => ability.index === abilityIndex,
    )!.ap;

    statusItem.offset += Math.floor(learnableAbilityIndex / 0x8);
    statusItem.bit = learnableAbilityIndex % 8;
...
export function overrideItem(item: Item): Item {
  if ("id" in item && item.id === "location") {
    ...
    return itemInt;
  } else if ("id" in item && item.id?.match(/gfAbility-/)) {
    const itemInt = item as ItemInt;

    const [index] = item.id.splitInt();

    const isLearned = getInt(itemInt.offset, "bit", { bit: itemInt.bit });

    if (isLearned) {
      itemInt.disabled = false;
    ...
    const offset = itemInt.offset - Math.floor(index / 0x8);

    gfAbilityList.forEach((ability) => {
      count += getInt(offset + Math.floor(ability.index / 0x8), "bit", {
        bit: ability.index % 8,
      });
    });

    itemInt.disabled = count >= 22;
    ...
    const offset = itemInt.offset + 0x21 + Math.floor(index / 0x8) - index;
...
export function afterSetInt(item: Item): void {
  ...
  } else if ("id" in item && item.id?.match(/^(item|magic)$/)) {
    const itemInt = item as ItemInt;
