...
export function overrideParseItem(item: Item): Item | ItemTab {
  if ("id" in item && item.id === "characterOutfits") {
    ...
    return itemSection;
  } else if ("id" in item && item.id === "battleIntel") {
...
export function getInventoryNames(itemType: number): Resource {
  ...
  [...Array(1000).keys()].forEach((index) => {
    const itemIndex = getInt(itemItem.offset + index * 0x18, "uint32");
...
export function getInventoryMateriaNames(type: string): Resource {
  ...
  [...Array(1000).keys()].forEach((index) => {
    ...
    const character = getInt(materiaItem.offset + index * 0x20 - 0x3, "uint8");
...
export function getInventoryWeaponNames(): Resource {
  ...
  [...Array(128).keys()].forEach((index) => {
    const weaponIndex = getInt(weaponItem.offset + index * 0x10, "uint32");

    if (weaponIndex !== 0x0) {
      names[index] = weapons[weaponIndex];
    }
  });
