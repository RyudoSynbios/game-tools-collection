...
export function overrideItem(item: Item): Item {
  ...
  if ("id" in item && item.id === "slots") {
    ...
    const maxLevel = getInt(itemInt.offset - index * 0x17d8 - 0x3f2b, "uint8");
...
export function afterSetInt(item: Item, flag: ItemBitflag): void {
  if ("id" in item && item.id === "unlockedModes") {
    ...
    const offset = itemInt.offset + index * 0x17d8 + 0x3f2b;

    let level = getInt(offset, "uint8");
    const maxLevel = getInt(itemInt.offset, "uint8");

    level = Math.min(level, maxLevel);

    setInt(offset, "uint8", level);
    setInt(itemInt.offset - 0x4, "uint8", level);
  } else if ("id" in item && item.id?.match(/experience-/)) {
    const itemInt = item as ItemInt;

    const [index] = item.id.splitInt();

    const maxLevel = getInt(itemInt.offset - index * 0x17d8 - 0x3f83, "uint8");
